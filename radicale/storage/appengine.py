...
class Collection(ical.Collection):
    ...
    def delete(self): 
        ...
        # IMPORTANT
        #
        #FIXME: ultimately (once the code has stabilized) we should try to be the less rirgorous we can
    ...
    # so we must handle them in the same function
    #
    # we will return the name of the object 
    # (because it might be generated by Radicale, eg, if we create a new vcard)
    def append(self, name, text_raw):
        return self.replace(name, text_raw)
    ...
    def replace(self, name, text_raw):
        ...
        for object in ItemContainerClass.parse_objects( text_raw ):
            ...
            if item.name:
                ...
                if not item_container: # the item does not exist, we are appending, we must create it
                    ...
                    # this will get called if this transaction succeeds
                    object_old = copy.deepcopy( item_container.object )
                    ndb.get_context().call_on_commit( lambda: hook_post_replace(collection_container, item_container, object_old) )
        ...
        collection_container.put()
        return item.name # return the name of the object so we know to access it later on
...
def hook_post_replace(collection_container, item_container, old_object):
